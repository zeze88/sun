...
const editCommentDB = ( commentId, title, comment ) => {
    return  function (dispatch, getState, { history }) {
        ...
        .put (`http://15.164.231.31/islogin/comment/revice/${commentId}`, {
            "commentTitle" : title,
            "comment" : comment,
        },
        {
...
const deleteCommentDB = ( commentId ) => {
    return  function (dispatch, getState, { history }) {
        ...
        .delete (`http://15.164.231.31/islogin//islogin/comment/delete/${commentId}`,
        {
            headers: {
                Authorization: token
            },
...
export default handleActions (
    ...
    [COMMENT_EDIT] : (state, action) => produce(state, (draft) => {
